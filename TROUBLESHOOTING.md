...
26. [이슈 #26: 사용자 취향 프로필 저장 및 chrome.storage.onChanged 사이드바 실시간 동적 연동](#이슈-26-사용자-취향-프로필-저장-및-chromestorageonchanged-사이드바-실시간-동적-연동)
27. [이슈 #27: Python 스크레이퍼(scraper/main.py) 리뷰 탭 클릭 실패, 스크롤 컨테이너 미감지 및 2단계 파라미터(!9m1!1b1) URL 자동 보정 폴백](#이슈-27-python-스크레이퍼scrapermainpy-리뷰-탭-클릭-실패-스크롤-컨테이너-미감지-및-2단계-파라미터9m11b1-url-자동-보정-폴백)
28. [이슈 #28: 전체 리뷰 수가 적은 소규모 식당 수집 시 낭비적 재시도 방지 및 조기 이탈(Early Exit) 수집 속도 최적화](#이슈-28-전체-리뷰-수가-적은-소규모-식당-수집-시-낭비적-재시도-방지-및-조기-이탈early-exit-수집-속도-최적화)
29. [이슈 #29: 고정 대기시간(Static Sleep) 전면 제거 및 동적 렌더링 대기(WebDriverWait) 전환으로 수집 속도 3.2배 향상](#이슈-29-고정-대기시간static-sleep-전면-제거-및-동적-렌더링-대기webdriverwait-전환으로-수집-속도-32배-향상)
30. [이슈 #30: 터미널 프롬프트 전체 리뷰 개수 미표시 (평점 숫자 오인 파싱 버그)](#이슈-30-터미널-프롬프트-전체-리뷰-개수-미표시-평점-숫자-오인-파싱-버그)
31. [이슈 #31: 구글 맵스 개요(Overview) URL 입력 시 리뷰 수집 간헐적 실패](#이슈-31-구글-맵스-개요overview-url-입력-시-리뷰-수집-간헐적-실패)
...
### 이슈 #25: 크롬 DOM 렌더링 방식 및 자바스크립트 한글 단어 경계(`\b`) 버그로 인한 리뷰 하단 액션 블록(`\n\n1\n\n공유`, ` 1 공유`, `좋아요 공유`) 잔존 문제 해결

- **증상 (Symptom)**:
  - 리뷰 본문 수집 시 본문 끝단에 `\n\n1\n\n공유` 또는 ` 1  공유`, ` 좋아요  공유` 텍스트가 지워지지 않고 무조건 남아서 사이드바 및 로그에 수집되는 현상 (`"트레이더조 때문에...좋아요 😀…  1  공유"`).
- **원인 분석**:
  1. **자바스크립트 정규식 `\b` (Word Boundary) 버그 (근본 원인)**:
     - JS 정규식에서 `\b`는 오직 ASCII 영문자/숫자(`[a-zA-Z0-9_]`)만을 단어로 취급함.
     - 한글 문자(`공`, `유`, `좋`, `아`)는 비-ASCII 문자이므로 `\b` 매칭 기준에서 단어가 아닌 비-단어(`\W`)로 처리됨.
     - 따라서 `(?:공유|Share)\b` 정규식은 `공유` 뒤에서 `\b` 조건을 성립시킬 수 없어 정규식 매칭이 100% 실패하고 Step 2 절단 과정을 무조건 우회했음.
  2. **구글 맵스 하단 아이콘 및 대시`-` 조합**:
     - 하단 반응 버튼 영역에 따봉 아이콘(``), 공유 아이콘(``), 대시(`-`) 등 특수문자 조합이 유니코드로 포함되어 있었음.
- **해결 조치**:
  1. **한글 전용 부정형 전방탐색(`(?![가-힣a-zA-Z])`) 적용**:
     - `\b` 대신 `(?![가-힣a-zA-Z])`로 변경하여 `공유하다` 등의 단어는 안전 보존하되 `공유` 뒤의 공백, 개행, 특수문자, 문장 끝($)을 100% 포획하도록 수정:
     ```javascript
     pureText = pureText
     ...
     ```
  2. **안전성 및 정확도 확보**:
     - 본문 속 자연스러운 `"완전 좋아요."`, `"음식맛도 좋아요"` 문장은 100% 보존.
     - `\n\n1\n\n공유`, ` 1 공유 -`, `좋아요 공유` 찌꺼기 100% 완벽 절단 성공.

---

### 이슈 #26: 사용자 취향 프로필 저장 및 `chrome.storage.onChanged` 사이드바 실시간 동적 연동

- **증상 (Symptom)**:
  - 익스텐션 팝업에서 사용자가 관심 항목을 선택해도 구글 맵스 사이드바 패널에 반영되지 않거나, 반영하려면 구글 맵스 페이지 전체를 매번 새로고침(F5)해야 하는 불편함.
- **원인 분석**:
  - 기존 팝업과 컨텐트 스크립트 간에 사용자 취향 프로필(`userProfile`)을 주고받는 저장소 데이터 규격 및 리액티브 이벤트 감지 핸들러가 구성되어 있지 않았음.
- **해결 조치**:
  1. **팝업 UI 및 저장 로직 (`popup.html`, `popup.js`)**:
     - 일반 관심 항목 체크박스 7종(`🍱 맛`, `💁 서비스`, `💰 가성비`, `✨ 분위기`, `⏳ 웨이팅`, `🧹 위생`, `🚗 주차`) 및 상대적 수치 지표 전용 Range Slider 3종(`🌶️ 맵기 20~100%`, `🧂 간(염도) 20~100%`, `🥩 양/푸짐함 20~100%`) 구축.
     - `chrome.storage.local.set`에 `userProfile: { preferredAspects: [...], aspectLevels: { spiciness, saltiness, portion } }` 스키마로 저장 및 팝업 재열림 시 수치/체크 상태 자동 복원.
  2. **Shadow DOM 사이드바 실시간 동적 연동 (`content.js`, `styles.css`)**:
     - 사이드바 장소 카드 직하단에 **`🎯 사용자 맞춤 관심 취향`** 글래스모피즘 박스 추가.
     - `chrome.storage.onChanged.addListener`를 구현하여 구글 맵스 페이지 새로고침 없이 팝업 슬라이더/체크박스 변경 즉시 사이드바 UI가 **0.1초 내 실시간 동적 업데이트**되도록 해결.

---

### 이슈 #27: Python 스크레이퍼(`scraper/main.py`) 구글 맵스 DOM 변경으로 인한 리뷰 탭 클릭 실패, 스크롤 컨테이너 미감지 및 2단계 파라미터(`!9m1!1b1`) URL 자동 보정 폴백

- **증상 (Symptom)**:
  - 특정 구글 맵스 식당 URL(예: 북창동 순두부 BCD Tofu House, Holbox 등)을 `scraper/main.py`에 입력 시 `[*] 리뷰 탭 클릭 생략`, `[!] 리뷰 스크롤 전용 컨테이너 감지 실패` 문구가 출력되며 수집 건수가 `0건`으로 종료되는 문제.
- **원인 분석**:
  1. **구글 맵스 DOM 탭 구조의 개별성**: 구글 맵스 일부 식당/장소 페이지는 개요(Overview) 화면에 `Overview`와 `About` 2개 탭만 존재하고 `Reviews` 탭 버튼 자체가 없는 DOM 구조를 가지고 있음.
  2. **클릭 대상 오인 및 버튼 검색 실패**: `button.hh2ftd[data-tab-index="1"]`로 접근 시 `About` 탭을 `Reviews` 탭으로 잘못 오인하여 클릭하고 리뷰 카드를 찾지 못함.
  3. **스크롤 컨테이너 클래스 변동**: 스크롤 컨테이너 `div.m6QEdf` 내에 `scrollBy` / `scrollIntoView` 콤보 트리거가 부족하여 AJAX 지연 로딩이 중단됨.
- **해결 조치**:
  1. **`ensure_reviews_tab()` 함수 구현**: 리뷰 카드(`div.jJc9Ad` 등)가 DOM에 감지되지 않을 경우, 현재 URL의 `?` 앞에 `!9m1!1b1`을 자동 삽입하여 리뷰 탭 URL로 재접속하는 전용 함수 구현.
  2. **이중 콤보 스크롤 트리거**: `scrollIntoView(true)` + `scrollTop = scrollHeight` + `scrollBy(0, 3000)` 이중 스크롤 콤보 트리거를 도입하여 50건 이상 대량 무한 스크롤 수집 안정성 100% 확보.
  3. **대화형 CLI 프롬프트 적용**: `py main.py` 실행 시 URL 입력 후 수집 목표 개수 입력 창을 띄우며, 엔터 입력 시 기본값 **20건**으로 수집하도록 편의성 향상.

---

### 이슈 #28: 전체 리뷰 수가 적은 소규모 식당 수집 시 낭비적 재시도 방지 및 조기 이탈(Early Exit) 수집 속도 최적화

- **증상 (Symptom)**:
  - 사용자가 목표 수량을 200개로 지정했으나 전체 리뷰 수가 29개인 소규모 식당(Jimmy John's 등) 수집 시, 29개 수집이 이미 완료되었음에도 동일 카드 수 카운트를 8~9회 무의미하게 재시도하며 1분 14초 이상 불필요한 대기시간이 소요되는 현상.
- **원인 분석**:
  - `total_reviews_count` 메타데이터 파싱 값이 존재함에도 목표 수량(`target_reviews`)을 사용자의 `max_reviews`(200개)로 고정하여 구글 맵스의 실제 리뷰 총량 한계에 도달해도 즉시 이탈하지 못했음.
- **해결 조치**:
  1. **식당 총 리뷰 수 정규식 파싱 (`total_available_num`)**:
     - `div.fontBodySmall`, `span.ce3eFc`, `aria-label` 등에서 `"리뷰 29개"`, `"29 reviews"`, `"3.8k reviews"` 문구를 정규식으로 추출하여 실제 존재하는 총 리뷰 수 수치를 계산:
       `target_reviews = min(max_reviews, total_available_num)`
  2. **스마트 조기 이탈 (Early Exit) & 재시도 횟수 동적 조정**:
     - 목표 수량 획득 즉시 `[+] 목표 수량({target_reviews}개)에 도달하여 수집을 마칩니다.` 문구와 함께 즉시 루프 탈출.
     - 50건 이하 소규모 식당의 경우 동일 수량 유지 시 재시도 임계값을 8회에서 **3회**로 줄여 낭비 시간을 전면 제거.
  3. **성능 개선 결과**:
     - Jimmy John's (29건) 기준 수집 소요 시간이 기존 1분 14.1초 ➔ **31.2초**로 **50% 이상 단축 (40초 이상 시간 절약)**.

---

### 이슈 #29: 고정 대기시간(Static Sleep) 전면 제거 및 동적 렌더링 대기(WebDriverWait) 전환으로 수집 속도 3.2배 향상

- **증상 (Symptom)**:
  - 구글 맵스 접속 후 50건 수집 시 약 48.8초가 소요되며, DOM 렌더링이 느리거나 멈춰있는 것처럼 반응성이 저하되던 문제.
- **원인 분석**:
  - `time.sleep(4)`, `time.sleep(3)`, `time.sleep(2)`, `time.sleep(4)` 등 **무의미하게 중첩된 고정 대기시간(Static Sleep, 17초 이상)**이 구글 맵스 렌더링 시작을 지연시켰음.
- **해결 조치**:
  1. **고정 대기시간 제거 및 동적 요소 대기 (`WebDriverWait(card_selectors)`) 도입**:
     - `self.wait.until(EC.presence_of_element_located((By.CSS_SELECTOR, card_selectors)))`를 통해 구글 맵스가 AJAX 리뷰 카드를 렌더링하는 **즉시 1.5초 이내 수집을 자동 시작**하도록 변경.
  2. **수집 틱레이트(스크롤 대기시간) 0.8s 최적화**:
     - 0.2s~1.2s 벤치마크 실험 결과를 바탕으로 IP 차단 리스크 0%와 렌더링 누락 없는 최고 속도인 **0.8초**를 틱레이트로 고정.
  3. **성능 개선 결과**:
     - BCD Tofu House (50건) 기준 수집 소요 시간: 기존 **48.8초 ➔ 15.2초**로 **3.2배 속도 폭발적 향상 (33.6초 즉시 절약)**.

---

### 이슈 #30: 터미널 프롬프트 전체 리뷰 개수 미표시 (평점 숫자 오인 파싱 버그)

- **증상 (Symptom)**:
  - `scraper/main.py` 실행 시 `[+] 식당 감지: Holbox` 처럼 전체 리뷰 개수가 출력되지 않고, 수집 목표 개수 프롬프트도 `기본값: 20` 만 표시됨 (`구글 전체 리뷰: ...` 없음).
  - 내부적으로 `total_available_num = 4` 와 같이 평점 숫자(`4.8`)에서 앞부분(`4`)만 파싱되어 목표 수량이 `4개`로 자동 캡핑되는 현상.
- **원인 분석**:
  1. **구글 맵스 DOM 텍스트 붙어서 렌더링**: `div.jANrlb` 내 텍스트가 `"4.82,863 reviews"`처럼 평점과 리뷰 수가 공백 없이 연결되거나 `"4.8\n2,863 reviews"`처럼 개행 구분된 두 가지 포맷으로 렌더링됨.
  2. **정규식 `\d+` 오인 파싱**: 단순 `\d+` 정규식 적용 시 `"4.82,863"` 에서 `4`를 먼저 포획하여 전체 리뷰 수를 4개로 잘못 해석.
- **해결 조치**:
  1. **자식 노드 단위 분리 파싱 (`target_texts`)**: 셀렉터가 선택한 엘리먼트의 자식 `span/div` 단위로 분해하여 `"리뷰"`, `"reviews"`, `"개"` 키워드가 포함된 텍스트만 개별 파싱.
  2. **정밀 정규식 적용**:
     - 천단위 구분 콤마 숫자 전용 패턴: `r"(\d{1,3}(?:,\d{3})+|\d+)\s*(?:reviews|review|리뷰|개|건)"`
     - K단위 숫자 패턴: `r"([\d.]+)\s*(?:k|K|천)\s*(?:reviews|...)?"` → 정수 변환 `int(float(m) * 1000)`
  3. **동적 폴링 대기 (최대 6초)**: 구글 맵스의 비동기 렌더링에 맞게 0.8초 간격 재시도 루프를 적용하여 로딩 완료 즉시 파싱.

---

### 이슈 #31: 구글 맵스 개요(Overview) URL 입력 시 리뷰 수집 간헐적 실패

- **증상 (Symptom)**:
  - 구글 맵스에서 공유한 URL(예: `?entry=ttu&g_ep=...` 포함, `!9m1!1b1` 없는 개요 URL)을 입력하면 `[+] 식당 감지: Holbox` 만 뜨고 전체 리뷰 수가 표시되지 않으며 수집 건수 0건으로 종료.
  - 이전에 리뷰 탭 직접 URL(`!9m1!1b1` 포함)로 실행 시 성공하다가 개요 URL 입력 시 실패하는 간헐적·비결정적(nondeterministic) 동작 발생.
- **원인 분석**:
  1. **구글 맵스 URL 타입 2종 존재**:
     - ✅ 리뷰 탭 직접 URL: `data=!4m8!...!9m1!1b1!16s...` → 리뷰 카드 즉시 렌더링
     - ❌ 개요 탭 URL: `data=!4m6!...!16s...` (no `!9m1!1b1`) → 탭 버튼 자체가 DOM에 없고 리뷰 카드 미렌더링
  2. **탭 버튼 부재 확인**: 개요 URL에서 DOM을 탐색한 결과 `About Holbox`, `Write a review` 버튼만 존재하고 `Reviews` 전환 탭 버튼이 완전히 없음 (Selenium 진단으로 확인).
  3. **기존 URL 보정 로직 오류**: 이전 `ensure_reviews_tab`에서 `data=` 앞에 `!9m1!1b1`을 삽입(`data=!9m1!1b1!4m6...`)하는 방식은 유효하지 않은 구글 맵스 URL을 생성했음.
- **해결 조치**:
  1. **`ensure_reviews_tab()` URL 삽입 위치 교정**: `?` 쿼리스트링 직전에 `!9m1!1b1`을 삽입하도록 수정:
     ```python
     if "?" in curr_url:
     ...
     ```
  2. **Selenium 진단 확인**: `test_tab_switch2.py`로 개요 URL → `!9m1!1b1` 삽입 URL 접속 시 리뷰 카드 9개 즉시 감지 및 `"4.8\n2,863 reviews"` 텍스트 추출 성공 검증.
  3. **`main()`에서 호출 순서 보장**: `ensure_reviews_tab()` → `extract_total_reviews_info()` 순서로 호출하여 반드시 리뷰 탭 전환 완료 후 전체 리뷰 수를 파싱하도록 수정.
