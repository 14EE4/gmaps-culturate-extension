...
- **해결 조치**:
  1. **1단계 (버튼 명시적 키워드 검증)**:
     - `button[aria-label*='reviews']`, `button[role='tab']`, `button.hh2ftd` 등 후보 버튼 수집 후 텍스트/라벨에 `"리뷰"`, `"Reviews"`, `"review"` 가 명시적으로 포함된 경우에만 탭 클릭을 수행하여 `About`/`Overview` 오클릭 100% 방지.
  2. **2단계 (핵심 URL 자동 보정 폴백)**:
     - 1단계 클릭 후에도 리뷰 카드(`div.jJc9Ad` 등)가 0개로 감지될 경우, 구글 맵스 리뷰 전용 접속 파라미터인 `!9m1!1b1`을 URL에 **자동 보정 삽입하여 즉시 재접속**하는 2단계 폴백 엔진 구현 (`print("[+] 리뷰 전용 파라미터(!9m1!1b1)로 URL 자동 보정 재접속 중...")`).
  3. **3단계 (이중 콤보 스크롤 트리거)**:
     - `scrollIntoView(true)` + `scrollTop = scrollHeight` + `scrollBy(0, 3000)` 이중 스크롤 콤보 트리거를 도입하여 50건 이상 대량 무한 스크롤 수집 안정성 100% 확보.
  4. **대화형 CLI 프롬프트 적용**:
     - `py main.py` 실행 시 URL 입력 후 수집 목표 개수 입력 창을 띄우며, 엔터 입력 시 기본값 **4건**으로 수집하도록 편의성 대폭 향상.
...
- **해결 조치**:
  1. **고정 대기시간 정단 및 동적 요동 대기 (`WebDriverWait(card_selectors)`) 도입**:
